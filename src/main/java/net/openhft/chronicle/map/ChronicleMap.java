 ...
 *
 * <p>For information on <ul> <li>how to construct a {@code ChronicleMap}</li> <li>{@code ChronicleMap}
 * flavors and properties</li> <li>available configurations</li> </ul> see {@link AbstractChronicleMapBuilder}
 * documentation.
 *
 * <p>Functionally this interface defines some methods supporting garbage-free off-heap programming: {@link
 * #getUsing(Object, Object)}, {@link #acquireUsing(Object, Object)}.
 *
 * <p>Roughly speaking, {@code ChronicleMap} compares keys and values by their binary serialized form, that
 * shouldn't necessary be the same equality relation as defined by built-in {@link Object#equals(Object)}
 * method, which is prescribed by general {@link Map} contract.
 *
 * <p>Note that {@code ChronicleMap} extends {@link Closeable}, don't forget to {@linkplain #close() close}
...
public interface ChronicleMap<K, V> extends ConcurrentMap<K, V>, ChronicleHash {
     ...
     * for the key.
     *
     * <p>If the value class allows reusing, particularly if it is a {@link Byteable} subclass, consider
     * {@link #getUsing(Object, Object)} method instead of this to reduce garbage creation.
     *
     * @param key the key whose associated value is to be returned
     * @return the value to which the specified key is mapped after this method call, or {@code null} if no
     * value is mapped
    ...
    /**
     * Returns the value to which the specified key is mapped, read to the provided {@code value} object, if
     * possible, or returns {@code null}, if this map contains no mapping for the key.
     *
     * <p>If the specified key is present in the map, the value data is read to the provided {@code value}
     * object via value marshaller's {@link BytesMarshaller#read(Bytes, Object) read(Bytes, value)} or value
     * reader's {@link BytesReader#read(Bytes, long, Object) read(Bytes, size, value)} method, depending on
     * what deserialization strategy is configured on the builder, using which this map was constructed. If
     * the value deserializer is able to reuse the given {@code value} object, calling this method instead of
     * {@link #get(Object)} could help to reduce garbage creation.
     *
     * <p>The provided {@code value} object is allowed to be {@code null}, in this case {@code
     * map.getUsing(key, null)} call is semantically equivalent to simple {@code map.get(key)} call.
     *
     * @param key        the key whose associated value is to be returned
    ...
    /**
     * The method is similar {@link  V getUsing(K key, V usingValue);}  but in addition read locks the map
     ...
     * {@code value} object, if possible, or returns {@code null}, if this map contains no mapping for the
     * key.
     *
     * <p>If the specified key is present in the map, the readContext.value() uses the provided {@code
     * usingValue} object via value marshaller's {@link BytesMarshaller#read(Bytes, Object) read(Bytes,
     * value)} or value reader's {@link BytesReader#read(Bytes, long, Object) read(Bytes, size, value)}
     ...
     * deserialized just as during {@link #getUsing(Object, Object) getUsing(key, usingValue)} call, passed
     * the same {@code key} and {@code usingValue} as into this method call. This means, as in {@link
