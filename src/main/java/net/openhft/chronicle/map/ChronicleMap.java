...
/**
 * Extension of {@link ConcurrentMap} interface, stores the data off-heap. <p> <p>For information on
 * <ul> <li>how to construct a {@code ChronicleMap}</li> <li>{@code ChronicleMap} flavors and
 * properties</li> <li>available configurations</li> </ul> see {@link ChronicleMapBuilder}
 * documentation. <p> <p>Functionally this interface defines some methods supporting garbage-free
 * off-heap programming: {@link #getUsing(Object, Object)}, {@link #acquireUsing(Object, Object)}.
 * <p> <p>Roughly speaking, {@code ChronicleMap} compares keys and values by their binary serialized
 * form, that shouldn't necessary be the same equality relation as defined by built-in {@link
 * Object#equals(Object)} method, which is prescribed by general {@link Map} contract. <p> <p>Note
 * that {@code ChronicleMap} extends {@link Closeable}, don't forget to {@linkplain #close() close}
...
public interface ChronicleMap<K, V> extends ConcurrentMap<K, V>, ChronicleHash {
     ...
     * no mapping for the key. <p> <p>If the value class allows reusing, particularly if it is a
     * {@link Byteable} subclass, consider {@link #getUsing(Object, Object)} method instead of this
     * to reduce garbage creation.
     *
     ...
     * <p> <p>If the specified key is present in the map, the value data is read to the provided
     ...
     * the given {@code value} object, calling this method instead of {@link #get(Object)} could
     * help to reduce garbage creation. <p> <p>The provided {@code value} object is allowed to be
     * {@code null}, in this case {@code map.getUsing(key, null)} call is semantically equivalent to
     * simple {@code map.get(key)} call.
     *
     * @param key        the key whose associated value is to be returned
     ...
     * the map segment to operations to be performed atomically. ( see the example below ) <p>
     ...
     * contains no mapping for the key. <p> <p>If the specified key is present in the map, the
     * readContext.value() uses the provided {@code usingValue} object via value marshaller's {@link
     * BytesMarshaller#read(Bytes, Object) read(Bytes, value)} or value reader's {@link
     * BytesReader#read(Bytes, long, Object) read(Bytes, size, value)} method, depending on what
     * deserialization strategy is configured on the builder, using which this map was constructed.
     * If the value deserializer is able to reuse the given {@code usingValue} object, calling this
     * method instead of {@link #get(Object)} could help to reduce garbage creation.
     * <pre>{@code
     ...
     * must be read while the segment lock is in place. <p> <p> <p>The provided {@code value} object
     * is allowed to be {@code null}, in this case
     *
     * @param key        the key whose associated value is to be returned
     * @param usingValue the object to read value data in, if possible
     * @return the read context containing the value to which the specified key is mapped <p> no
     * mapping for the key
     * @see #get(Object)
     * @see #getUsing(Object, Object)
     * @see ChronicleMapBuilder#valueMarshaller(BytesMarshaller)
    ...
    /**
     * Acquire a value for a key, creating if absent. <p> <p>If the specified key is absent in the
     * map, {@linkplain ChronicleMapBuilder#defaultValue(Object) default value} is taken or
     * {@linkplain ChronicleMapBuilder#defaultValueProvider(DefaultValueProvider) default value
     * provider} is called. Then this object is put to this map for the specified key. <p> <p>Then,
     * either if the key was initially absent in the map or already present, the value is
     ...
     * value instance is created to deserialize the data. <p> In code, {@code acquireUsing} is
     * specified as :
     * <pre>{@code
     ...
     * }}</pre>
     * <p> <p> Where {@code defaultValue(key)} returns either {@linkplain
     * ChronicleMapBuilder#defaultValue(Object) default value} or {@link
     * ChronicleMapBuilder#defaultValueProvider(DefaultValueProvider) defaultValueProvider.} <p>
    ...
    /**
     * A special operation, similar to {@link #replace(Object, Object, Object) replace(key, value,
     * value)}, but puts the entry even if it was absent in the map.
     */
    ...
    /**
     * Creates an empty value instance, which can be used with the following methods : <p> {@link
     * ChronicleMap#getUsing(java.lang.Object, java.lang.Object) } {@link
     * ChronicleMap#getUsingLocked(java.lang.Object, java.lang.Object)    } {@link
     * ChronicleMap#acquireUsing(java.lang.Object, java.lang.Object)      } {@link
     * ChronicleMap#acquireUsingLocked(java.lang.Object, java.lang.Object)  } <p> for example like
     * this : <p>
     * <pre>{@code
