...
/**
 * Staged context definition used to compile Chronicle-Map query pipelines for
 * replicated maps.
 * <p>
 * The top-level and nested stages declared in {@link Context} extend the
 * non-replicated {@link MapQueryContext} wiring with replication-specific
 * components such as tombstone handling, modification iterators and remote
 * update tracking. The class itself is a template: concrete query contexts are
 * generated by the staging framework and expose the {@link ReplicatedMapQuery}
 * API to higher layers.
 */
@Staged
@Context(topLevel = {
