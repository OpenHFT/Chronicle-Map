...
public class OffHeapByteArrayExampleTest {
    ...
    public void test() {
        ...
        Assertions.assertEquals(0, value.getByteValueAt(2).getValue(), "value.getByteValueAt(2).getValue()");
        Assertions.assertEquals(0, value.getByteValueAt(3).getValue(), "value.getByteValueAt(3).getValue()");

        byte actual = value.getByteValueAt(1).getValue();
        Assertions.assertEquals(EXPECTED, actual, "retrieved byte value should match the expected value after roundtrip");
