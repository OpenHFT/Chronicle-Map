...
class LostKeyTest {
    ...
    void lostkeysTest() throws Exception {
        ...
        try {
            ...
            byte[] h1add = decodeHex("591e91f809d716912ca1d4a9295e70c3e78bab077683f79350f101da6458807300000000");
            mmap.put(h1add, valueSample);

            byte[] bLostKey = decodeHex(lostKey);
            mmap.put(bLostKey, valueSample);

            LongValue v1 = mmap.remove(h1add);
            assertEquals(1L, v1.getValue());

            byte[] h3add = decodeHex("6f80ca7441710cf0942cf99e3e8aa59d38d73b124730306afeb1aec8f08fd76b00000000");
