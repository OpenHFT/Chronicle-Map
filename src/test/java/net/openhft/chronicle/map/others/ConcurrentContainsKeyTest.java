 ...
 * @summary Test that entries are always present in the map, when entries are held within one bin that is a
 * tree
 */
@Test
public class ConcurrentContainsKeyTest {
    ...
    // Should be > net.openhft.chronicle.map.ConcurrentHashMap.TREEIFY_THRESHOLD but small
    ...
    public void testContainsKey() {
        X[] content = IntStream.range(0, N).mapToObj(i -> new X(i)).toArray((java.util.function
                .IntFunction<X[]>) (value) -> new X[value]);
        // Create map with an initial size >= net.openhft.chronicle.map.ConcurrentHashMap.TREEIFY_THRESHOLD
