...
Chronicle Map Replication is part of Chronicle Queue Enterprise; which is commercially supported.
...
TCP/IP is a reliable protocol.
This means that, unless you have a network failure or hardware outage, the data is guaranteed to arrive.

== How Chronicle Map Replication works

An underlying Chronicle Queue is used to persist changes to the Map, and this is replicated using Chronicle Queue Enterprise replication to one or more other Chronicle Maps.
One underlying queue can be shared by multiple Maps, if required.
...
Replication occurs from source to sink and a slow consumer, or a dead sink, will not impact replication to other sinks or the source Map's performance.
...
If, for example the master host or data centre is changed, and it typically takes only milliseconds to reconfigure the cluster with a new source, and restart replication.
