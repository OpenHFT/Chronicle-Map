...
The _public API_ comprises public classes that are included in
...
`MapEntry`.
They must be added in such a way that any existing code, calling methods on those classes still compiles; and does not
...
In addition, potential _concurrent_ accessing implementations of older versions ("within" the same revision) should not lose correctness or corrupt the data.
