...
the data store is _in-process_ for each of the accessing processes.
Out-of-process approach to IPC
...
entries _off-heap_, serializing/deserializing key and value objects to/from off-heap memory
transparently.
...
happens.
This is under the control of the OS. All data is guaranteed to be written to disk when the Map is closed.
