...
methods.
Methods in the `MapMethods` interface correspond to `Map` interface methods with the same
...
. Query tier: `MapQueryContext` interface
. Entry tier: `MapEntry` and `MapAbsentEntry` interfaces
. Data tier: `Data` interface
