...
The easiest way to compute the size of the serialized form of some object of the
...
array length, plus the sum of sizes of all CharSequences, in UTF-8 encoding.
Computing this size
without actual encoding has comparable computational cost with performing actual encoding.
That
makes `CharSequence[]` type to meet the second criteria (see above).
...
minimizing the amount of produced garbage.
...
configurations (charset and input buffer size).
They are implemented as normal classes rather
